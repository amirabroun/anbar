<div class="an-card">
    <div class="an-toolbar">
        ...
        <a class="an-btn an-btn-primary an-btn-sm" href="create_memory.php" style="margin-right:14px">
    ...
    <div class="an-table-wrap">
        <table class="an-table" data-an-table data-an-page-size="10">
            ...
            if ($getMemory) {
                foreach ($getMemory as $key => $getMemory_item) {
                    $isActive = $getMemory_item['status'] === 'active';
                    ?>
                    <tr>
                        <td data-rank="1"><?php echo $key + 1 ?></td>
                        <td><span class="an-cell-title"><?php echo $getMemory_item['title'] ?>GB</span></td>
                        <td>
                            <span class="an-badge is-<?php echo $isActive ? 'success' : 'muted' ?>"><span class="an-dot"></span><?php echo $isActive ? 'فعال' : 'غیرفعال' ?></span>
                        </td>
                        <td>
                            <div class="an-actions">
                                <a class="an-iconbtn is-bolt" href="?action=change_status_memory&memory_id=<?php echo $getMemory_item['id'] ?>&old_status_memory=<?php echo $getMemory_item['status'] ?>" title="فعال / غیرفعال">
