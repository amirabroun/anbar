<div class="an-card">
    <div class="an-toolbar">
        ...
        <a class="an-btn an-btn-primary an-btn-sm" href="create_ram.php" style="margin-right:14px">
    ...
    <div class="an-table-wrap">
        <table class="an-table" data-an-table data-an-page-size="10">
            ...
            $getRam = selectramTBL();
            if ($getRam) {
                foreach ($getRam as $key => $getRam_item) {
                    $isActive = $getRam_item['status'] === 'active';
                    ?>
                    <tr>
                        <td data-rank="1"><?php echo $key + 1 ?></td>
                        <td><span class="an-cell-title"><?php echo $getRam_item['title'] ?>GB</span></td>
                        <td>
                            <span class="an-badge is-<?php echo $isActive ? 'success' : 'muted' ?>"><span class="an-dot"></span><?php echo $isActive ? 'فعال' : 'غیرفعال' ?></span>
                        </td>
                        <td>
                            <div class="an-actions">
                                <a class="an-iconbtn is-bolt" href="?action=change_status_ram&ram_id=<?php echo $getRam_item['id'] ?>&old_status_ram=<?php echo $getRam_item['status'] ?>" title="فعال / غیرفعال">
