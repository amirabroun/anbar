<div class="an-card">
    <div class="an-toolbar">
        ...
        <a class="an-btn an-btn-primary an-btn-sm" href="create_color.php" style="margin-right:14px">
    ...
    <div class="an-table-wrap">
        <table class="an-table" data-an-table data-an-page-size="10">
            ...
            if ($getcolor) {
                foreach ($getcolor as $key => $getcolor_item) {
                    $isActive = $getcolor_item['status'] === 'active';
                    ?>
                    <tr>
                        <td data-rank="1"><?php echo $key + 1 ?></td>
                        <td><span class="an-cell-title"><?php echo $getcolor_item['title'] ?></span></td>
                        <td>
                            <span class="an-badge is-<?php echo $isActive ? 'success' : 'muted' ?>"><span class="an-dot"></span><?php echo $isActive ? 'فعال' : 'غیرفعال' ?></span>
                        </td>
                        <td>
                            <div class="an-actions">
                                <a class="an-iconbtn is-edit" href="update_color.php?color_id=<?php echo $getcolor_item['id'] ?>" title="ویرایش">
                                    <svg class="an-ic"><use href="#an-i-edit"></use></svg>
                                </a>
                                <a class="an-iconbtn is-bolt" href="?action=change_status_color&color_id=<?php echo $getcolor_item['id'] ?>&old_status_color=<?php echo $getcolor_item['status'] ?>" title="فعال / غیرفعال">
