...
if (isset($_POST["action"]) && pageName() === "createArticles") {
    if ($_POST["action"] === "insert_paper") {
        ...
        if ($validate_fields['status']) {
            ...
            if (insertRecordToTable($tableName, $fields)) {
                setMessage('عملیات موفق', 'افزودن مقاله با موفقیت انجام شد', 'success');
            }
        }
    }
}
...
if (isset($_POST['action']) && $_POST['action'] === 'update_blog') {
    ...
    if ($validation['status']){
        $update_category = updateblogg($_POST['title'], $_POST['Created'], $_POST['description'],$_GET['blog_id']);
        $id1=$_POST["id"];
        if ($update_category) {
