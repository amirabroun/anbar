...
<div class="row mb-3 mx-0 px-res-0 ">
    ...
    $inter_user = selectInterestByUserId($user_id['id']);
    ?>

    <?php
    if ($inter_user){
    foreach ($inter_user as $product){
        ...
        foreach ($inter as $inter_user_one){
            ...
            <div class="col-lg-3 col-md-4 col-sm-6 col-12 px-10 mb-1 px-res-0">
                <div class="product-card mb-2 mx-res-0">
                    ...
                    else if (!empty($inter_user_one['price_discounted'])) {
                        ?>
                        <div class="discount">
                                                    <span><?php $cal_percentage = $inter_user_one['price'] - ($inter_user_one['price_discounted']);
                                                        echo cal_percentage($cal_percentage, $inter_user_one['price']) . '%<br/>'; ?></span>
                        </div>
                        <br>
                        <br>
                        <?php
                    ...
                    ?>
                    <a class='product-thumb' target="_blank" href='<?php echo productUrl($inter_user_one['tracking_code'])?>'>
                        <?php

                        if (!empty($inter_user_one['photo_name'])){
                            ?>
                            <img  height="100" width="100" src='<?php echo normalizedPath(DOMAIN['public'], $inter_user_one['photo_src'], $inter_user_one['photo_name'])?>' alt='<?php echo $inter_user_one['title']?>'>
                            <?php
                        }else{
                            ?>
                            <img  height="120" width="150" src='<?php echo normalizedPath(DOMAIN['public'],'/images/180.png')?>' alt='<?php echo $inter_user_one['title']?>'>
                            <?php
                        }
                        ?>
                    </a>
                    <div class="product-card-body">
                        <h5 class="product-title">
                            <a href='<?php echo productUrl($inter_user_one['tracking_code'])?>'><?php echo $inter_user_one['title']?></a>
                        </h5>

                        <span class='product-price'>
                  <span class='product-price'>
                                        <?php
                                        if (empty($inter_user_one['price_discounted']))
                                        {
                                            ?>
                                            <strong><?php echo priceFormant($inter_user_one['price'])?> </strong>
                                            <?php
                                        } else
                                        {
                                            ?>
                                            <strong class="text-danger"><?php echo priceFormant($inter_user_one['price_discounted'])?> </strong>
                                            <br>
                                            <del><?php echo priceFormant($inter_user_one['price'])?></del>
                                            <?php
                                        }
                                        ?>
...
<div class="row d-flex justify-content-center align-items-center mt-3">
                                    ...
                                    <div class="addCartIndex ml-1" data-product-id="<?php echo $inter_user_one['tracking_code']?>">
                                        <!--<input type="hidden" name="action" value="add_to_cartIndex">-->
                                        <input type="hidden" name="product"
                                               value="<?php echo $inter_user_one['tracking_code']; ?>">
                                    ...
                                    <div class="addInterestIndex ml-1" data-product-id="<?php echo $inter_user_one['id']?>">
                                        <input type="hidden" name="action" value="interest">
                                        <?php
                                        if (isset($_SESSION['user_sing'])) {
                                            $user_id = getIdUsers($_SESSION['user_sing']);
                                            $single_id = $inter_user_one['id'];
                                            ...
                                            if ($select_one_product) {
                                                ...
                                                <button
                                                        ...
                                                        value="<?php echo $inter_user_one['id'] ?>">
                                            ...
                                            <button type="submit" class="btn btn-danger btn-Add-interest-Index"
                                                    name="id" value="<?php echo $inter_user_one['id'] ?>">
                                                <svg style="margin-top: 3px" xmlns="http://www.w3.org/2000/svg"
                                    ...
                                    <a class="btn btn-success pointer-event" data-toggle="modal"
                                       data-target="#exampleModal<?php echo $inter_user_one['id'] ?>">
